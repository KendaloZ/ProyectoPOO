...
public class CrearMedicamentoViewController {
    ...
    private void guardarMedicamento(ActionEvent event) {
        if (validarCampos()) {
            Medicamento nuevoMeedicamento = new Medicamento();
            nuevoMeedicamento.setNombre(txtNombre.getText());
            nuevoMeedicamento.setDisponible(cbDisponible.getValue());

            boolean exito = medicamentoController.insertarMedicamento(nuevoMeedicamento);

            if (exito) {
                mostrarAlerta(Alert.AlertType.INFORMATION, "Éxito", "Medicamento guardado correctamente");
                limpiarCampos();
            } else {
                mostrarAlerta(Alert.AlertType.ERROR, "Error", "No se pudo guardar el mediacamento");
            }
        }
    }
